  ...
  return (
    <section className="py-24 px-6 md:px-14 max-w-6xl mx-auto">
      <div className="mb-14">
        ...
        >
          How World Bridge Capital<br/>generates your <em className="italic text-hunter font-medium">returns.</em>
        </motion.h2>
      </div>

      <motion.div 
        initial="hidden" whileInView="visible" viewport={{ once: true }} variants={reveal}
        className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 border border-ink-200 shadow-sm"
      >
        {steps.map((step, i) => (
          <div 
            key={i} 
            className="p-8 border-b lg:border-b-0 lg:border-r border-ink-200 bg-white relative group hover:bg-cream-100 transition-colors duration-300"
          >
            {/* Top hover indicator */}
            <div className="absolute top-0 left-0 right-0 h-[2px] bg-transparent group-hover:bg-hunter transition-colors duration-300"></div>
            
            <div className="font-mono text-[10px] font-medium tracking-[0.16em] uppercase text-ink-400 mb-6">
              {step.tag}
            </div>
            <step.icon className="w-6 h-6 text-ink-800 mb-4 stroke-[1.5]" />
            ...
            <div className="font-mono text-[10px] text-hunter mt-6 tracking-[0.06em]">
              {step.cycle}
            </div>
          </div>
        ))}
