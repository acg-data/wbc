...
interface EsVSLProps {
  onQuizClick: () => void;
}

export function EsVSL({ onQuizClick }: EsVSLProps) {
    ...
    <div className="relative bg-indigo-dark border-y border-indigo shadow-2xl mt-8">
            ...
            >
              ...
              <div className="text-center px-4">
                <span className="block text-lg md:text-xl font-bold text-white tracking-wide">
                  Por Qué Las Finanzas Comerciales Son La Clase de Activo Que Los Inversores Sofisticados Están Adoptando
                </span>
                <span className="block text-xs font-medium text-white/50 tracking-[0.15em] uppercase mt-3">
                  6 minutos · Solo inversores acreditados · Sin obligación
                </span>
              </div>
            </motion.div>
          )}
      ...
      <div className="bg-indigo-dark border-t border-white/5 px-6 md:px-14 py-5 flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="text-sm font-medium text-white/60">
          ¿Listo para verificar tu elegibilidad? <strong className="text-white">Realiza el cuestionario de inversor de 90 segundos a continuación.</strong>
        </div>
        <button 
          onClick={onQuizClick}
        ...
        >
          Verificar Mi Elegibilidad &rarr;
        </button>
      </div>
    </div>
  );
