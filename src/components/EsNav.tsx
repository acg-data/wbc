...
export function EsNav({ onQuizClick }: NavProps) {
  return (
    <>
      ...
      <a href="#quiz" className="sr-only focus:not-sr-only focus:absolute focus:top-2 focus:left-2 focus:z-[9999] focus:bg-teal-dark focus:text-white focus:px-4 focus:py-2 focus:text-sm">
        Saltar al cuestionario de elegibilidad
      </a>
      ...
      <div className="bg-indigo-dark text-teal-pale text-center py-2.5 px-6 text-xs font-semibold tracking-widest uppercase" role="banner">
        Asignación Q2 2026 ahora aceptando solicitudes — <strong className="text-white">Para inversores acreditados</strong>
      </div>
      
      <nav className="sticky top-0 z-50 h-16 flex items-center justify-between px-6 md:px-14 bg-cream/95 backdrop-blur-md border-b border-indigo/10 shadow-sm" aria-label="Navegación principal">
        <a href="/es" className="flex items-center gap-3" aria-label="World Bridge Capital - Inicio">
          ...
          <div className="flex flex-col">
            ...
            <span className="font-sans text-[8px] font-bold tracking-[0.2em] uppercase text-teal-dark mt-0.5">Plataforma de Inversión en Finanzas Comerciales</span>
          </div>
        </a>
        
        <div className="flex items-center gap-4 md:gap-6">
          <a href="/" className="text-xs font-medium text-ink-400 hover:text-indigo transition-colors">
            EN
          </a>
          <button 
            onClick={onQuizClick}
