...
export function FinalCTA({ onQuizClick }: FinalCTAProps) {
  ...
  return (
    <section className="bg-indigo-dark py-24 px-6 md:px-14 relative overflow-hidden text-center border-t border-indigo">
      ...
      <div className="max-w-3xl mx-auto relative z-10">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={reveal}>
          <div className="flex items-center justify-center gap-3 text-[10px] font-bold tracking-[0.2em] uppercase text-teal mb-6">
            <div className="w-5 h-px bg-teal opacity-70"></div>
        ...
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={reveal} className="flex flex-col items-center">
          ...
          <div className="mt-12 p-5 bg-teal/10 border border-teal/20 max-w-lg flex items-start gap-4 text-left">
            ...
            <p className="text-[12px] text-white/50 leading-[1.6]">
              <strong className="text-teal font-semibold">Confidential & No Obligation:</strong> Your information is never shared with third parties. Completing the quiz or application does not constitute an investment commitment. Past results do not guarantee future returns. Not an offer to sell securities.
            </p>
          </div>
        </motion.div>
      </div>
